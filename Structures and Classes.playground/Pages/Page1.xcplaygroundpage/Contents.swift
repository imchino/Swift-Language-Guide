 ...
 */
/*:
 構造体とクラスは、プログラムのコードの構成要素となる汎用の柔軟な構成体です。
 構造体およびクラスに機能を追加するには、「定数、変数、および関数」を定義するのに使用するのと同じ構文を使用して、プロパティとメソッドを定義します。

 Swiftは他のプログラミング言語とは異なり、カスタム構造体やクラス用に個別のインターフェースファイルと実装ファイルを作成しません。
 Swiftは、構造体またはクラスをファイルに定義すると、そのクラスまたは構造体への外部インタフェースが自動的に他のコードから使用できるようになります。
 
 ### NOTE
 クラスのインスタンスは伝統的にオブジェクトとして知られています。
 ただし、他の言語と比較すると、Swiftの構造体とクラスは機能的にはるかに近いため、この章の大部分では「クラスまたは構造体」型のインスタンスに適用される機能について説明します。
 このため、より一般的な用語としてインスタンスが使用されます。
 */
/*:
